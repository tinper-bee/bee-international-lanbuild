...
axios.interceptors.response.use(function (response) {
    ...
    if(/4\d{2}/.test(error.response.data.status)){
        Message.create({content: '$i18n{index.js0}$i18n-end', color: 'danger', duration: null})
    }else if(/5\d{2}/.test(error.response.data.status)){
        Message.create({content: '$i18n{index.js1}$i18n-end', color: 'danger', duration: null})
    }else{
        Message.create({content: '$i18n{index.js2}$i18n-end', color: 'danger', duration: null})
    }
